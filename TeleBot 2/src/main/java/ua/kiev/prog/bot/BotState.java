...
public enum BotState {
    ...
    EnterAge {
        ...
        @Override
        public void handleInput(BotContext context) {
            String ageStr = context.getInput();
            try {
                context.getUser().setAge(Integer.parseInt(ageStr));
            } catch (NumberFormatException e) {
                sendMessage(context, "Enter only numbers, please!");
            }
            if (context.getUser().getAge() == null)
                next = EnterAge;
            else if (context.getUser().getAge() < 18) {
